...
public:
  Constraint(
      ...
      : _id(id), _projection(std::move(projection)), _nodes(nodes) {}
  ...
  void projectNodePositions() {
    ...
    for (uint32_t i = 0; i < NodeCount; ++i) {
      this->_nodes[i]->position = fixedPositions[i];
    }
  }

  const Node& getNode(uint32_t nodeIndex) const {
    if (nodeIndex >= NodeCount) {
      throw std::runtime_error("Invalid nodeIndex given to Constraint::getNode.");
    }
